...
class shortMPSOptimizer(object):
    ...
    def train(self, data_source, batch_size, n_step, optional_parameters=MPSTrainingParameters()):
        ...
        with tf.Session() as sess:
            ...
            for i in range(n_step):
                ...
                print(self.test[0], self.test[self.MPS.input_size-1])
    ...
    def _setup_optimization(self):
        ...
        with tf.name_scope("setup_optimization"):
            ...
            start_node = self.MPS.nodes.read(0) 
            start_node.set_shape([self.MPS.d_feature, None])
            C1 = tf.matmul(feature[0], start_node)
            ...
            end_node = self.MPS.nodes.read(self.MPS.input_size-1)
            end_node.set_shape([self.MPS.d_feature, None])
            C2 = tf.matmul(feature[self.MPS.input_size-1], end_node)
            ...
            _, _, self.C2s = tf.while_loop(cond=cond, body=self._find_C2, loop_vars=[self.MPS.input_size-2, C2, C2s],
                                           shape_invariants=[tf.TensorShape([]), tf.TensorShape([None, None]),
                                                             tf.TensorShape(None)],
                                           parallel_iterations=10,
                                           name="initialFindC2")
    ...
    def _find_C2(self, counter, prev_C2, C2s):
        ...
        C2s = C2s.write(counter-2, new_C2)
        counter = counter - 1
    ...
    def train_step(self):
        ...
        with tf.name_scope("train_step"):
            ...
            self.MPS._special_node_loc = self.MPS.input_size - 2
    ...
    def _sweep_left(self):
        ...
        assert self.MPS._special_node_loc == self.MPS.input_size - 2
        ...
        cond = lambda counter, *args: tf.greater(counter, 1)
        wrapped = [self.MPS.input_size-2, self.acc_lr_reg, self.C2s, self.updated_nodes, n1]
    ...
    def _duplicate_nodes(self, nodes, from_index, to_index):
        ...
        with tf.name_scope("duplicatingNodes"):
            ...
            from_index = tf.cond(tf.less(from_index, 1), lambda: 1, lambda: from_index)
