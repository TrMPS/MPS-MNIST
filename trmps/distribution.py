...
def quad_sample(a, b, tol, name='quadratic', seed=None):
    norm = tf.sqrt(a * a + b * b)
    _a = a / norm
    _b = b / norm
    cdfs = tf.random_uniform([tf.shape(a)[0]], minval=0, maxval=1, seed=seed)
    return _inverse_cdf(cdfs, _a, _b, tol)


def _inverse_cdf(f, _a, _b, _tol):
        size = tf.shape(f)[0]
        ...
        def inverse(counter, x_ta):
            ...
            a = _a[counter]
            cdf = f[counter]
            finite = tf.greater(tf.abs(b), _tol)
            x = tf.cond(finite, true_fn=lambda: _inverse_cdf_finite_b(cdf, a, b),
                                false_fn=lambda: _inverse_cdf_zero_b(cdf, a))
...
def _inverse_cdf_zero_b(f, a):
        return f / tf.square(a)


def _inverse_cdf_finite_b(f, a, b):
    c = a - np.sqrt(3) * b
...
class Quadratic(object):
    """
    A 'quadratic' distribution where pdf(x) = (a + \sqrt(3)(2x - 1) b)^2
    ...
    def _sample_test(self, seed=None):
        cdfs = tf.random_uniform([self._n_params], minval=0, maxval=1, seed=seed)
        return cdfs, self._inverse_cdf(cdfs)
...
if __name__ == '__main__':
    ...
    quad_dist = Quadratic(a, b)
    cdfs, samples = quad_dist._sample_test()
    cdfs_inv = quad_dist.cdf(samples)
