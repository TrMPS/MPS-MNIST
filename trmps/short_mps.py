...
#         self.nodes = updated_nodes
    ...
    def _setup_nodes(self):
        ...
        with tf.name_scope("MPSnodes"):
            ...
            start_node, sv = self._make_start_node()
            self._append_node(0, start_node, [self.d_feature, None])
            ...
            end_node, sv = self._make_end_node() 
            node = tf.einsum('mij,jk->mik', node, sv)
            self._append_node(self.input_size-2, node, shape)
            self._append_node(self.input_size-1, end_node, [self.d_feature, None])
    ...
    def _make_start_node(self):
        ...
        start_node = tf.Variable(start_vector, dtype=tf.float32,
                           trainable=False, name='start_node')
        s, u, v = tf.svd(start_node)
    ...
    def _make_end_node(self):
        ...
        end_node = tf.Variable(end_vector, dtype=tf.float32,
                           trainable=False, name='end_node')
        s, u, v = tf.svd(end_node)
    ...
    def predict(self, feature):
        ...
        with tf.name_scope("MPS_predict"):
            ...
            with tf.name_scope('calculate_C1'):
                ...
                start_node = self.nodes.read(0) 
                start_node.set_shape([self.d_feature, None])
                C1 = tf.matmul(self.feature[0], start_node)
            ...
            with tf.name_scope('calculate_C2'):
                end_node = self.nodes.read(self.input_size-1)
                end_node.set_shape([self.d_feature, None])
                C2 = tf.matmul(self.feature[self.input_size-1], end_node)
